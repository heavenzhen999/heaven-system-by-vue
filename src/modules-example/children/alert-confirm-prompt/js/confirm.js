...
export default function(content, title, callback) {
  try {
    let container = window.top
    // 判断同时有几个alert(confirm prompt 一起计算)在弹出状态
    // 一层最多存在的数量
    if (container.calertDefaultTop === undefined) container.calertDefaultTop = container.innerHeight * 0.1
    if (container.calertDefaultLeft === undefined) container.calertDefaultLeft = (container.innerWidth - 350) / 2
    if (container.calertNowTop === undefined) container.calertNowTop = container.innerHeight * 0.1
    if (container.calertNowLeft === undefined) container.calertNowLeft = (container.innerWidth - 350) / 2
    ...
    cconfirmButtonYes.style.cursor = 'pointer'
    ...
    cconfirmButtonNo.style.cursor = 'pointer'
