/*
 * @Author: chen zhen
 * @Date: 2018-09-18 14:26:18
 * @Last Modified by: chen zhen
 * @Last Modified time: 2018-09-18 14:41:35
 * @Description 一个canvas使用zrender渲染的圆盘时钟
 */
...
export default {
  ...
  computed: {
    hour() {
      return this.date !== null ? new Date(this.date).getHours() : 0
    },
    minutes() {
      return this.date !== null ? new Date(this.date).getMinutes() : 0
    },
    second() {
      return this.date !== null ? new Date(this.date).getSeconds() : 0
    }
  },
  watch: {
    minutes(newVal) {
